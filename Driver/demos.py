import numpy as np
from Driver.simulation_utils import create_env, get_feedback, run_algo, load_trajectories, get_feedback_no_sim
import pandas as pd
import time
...
def get_current_error(w_samples, sample_mode, alpha_samples, iter, user, solver, slider_step_size, trajectories, simulation_object,
    ...
    w_exp = np.divide(w_exp, sum(w_exp))
    # w_user = np.divide(user['w'], np.linalg.norm(user['w']))
    w_user = np.divide(user['w'], sum(user['w']))
    features_set = trajectories['feature_set']
    min_reward = np.dot(features_set[np.argmin(features_set @ w_user.T)], w_user)
    phi_user = get_trajectory_for_weight(simulation_object, w_user)['phi']
    # phi_user = features_set[np.argmax(features_set @ w_user.T)]
    # phi_exp = features_set[np.argmax(features_set @ w_exp.T)]
    phi_exp = get_trajectory_for_weight(simulation_object, w_exp)['phi']
    print('u(w^*) = ', np.dot(phi_user, w_user), 'f(w_est)*w^*',np.dot(phi_exp, w_user), np.dot(phi_exp, w_user)/np.dot(phi_user, w_user))
    print('f_est',phi_exp)
    print('f^*',phi_user)
    print('w^*', w_user)
    print('w_exp', w_exp)
...
def run(task, sample_mode, trajectories, resolutions, generated_users, acquisitions):
    """
    ...
    num_samples = 200
    identifier = task + '_'+sample_mode +'_' + str(round(time.time() * 100))  # id for the trial used as the filename
    df = pd.DataFrame()
    for user_id in range(len(generated_users)):  # repeat experiment for different users
        ...
        for acquisition in acquisitions:
            for resolution in resolutions:
                ...
                PQ = []
                Up = []
                w_samples = np.random.rand(num_samples, d)
                ...
                error_hist = get_current_error(w_samples, sample_mode, alpha_samples, 0, user, acquisition, resolution, trajectories,
                                               simulation_object, PQ_val, Up_val,
                                               validation_resolution)  # error logging
                print('initial error', error_hist)
                while i < 5:  # number of learning iterations
                    ...
                    curr_error = get_current_error(w_samples, sample_mode, alpha_samples, i, user, acquisition, resolution,
                                                    trajectories,
