...
def alpha_plots(df, measure='Alignment', sigma=0.1):
    ...
    for solver_idx in range(len(solvers)):
        for alpha_idx in range(len(alphas)):
            ...
            sns.lineplot(ax=ax, x='iter', y=measure, hue='sample_mode', hue_order=hueorder, data=df_tmp, ci=90)
...
def measures_plot_pairs(df, measure='alignment', sigma=0.1):
    ...
    for idx in range(len(solvers)):
        ...
        hueorder = None
        print(df_tmp)
        sns.lineplot(ax=axes[idx], x='iter', y=measure, hue='sample_mode', hue_order=hueorder, data=df_tmp, ci=90)
