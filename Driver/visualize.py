...
import Driver.car
import math
import Driver.dynamics
import Driver.utils_driving as utils
import matplotlib.cm
import theano as th
import Driver.feature
import pickle
import sys
...
class Visualizer(object):
    def __init__(self, dt=0.5, fullscreen=False, name='unnamed', iters=1000, magnify=1.):
        ...
        self.grass = pyglet.resource.texture('Driver/imgs/grass.png')
        ...
        def car_sprite(color, scale=0.15/600.):
            sprite = pyglet.sprite.Sprite(centered_image('Driver/imgs/car-{}.png'.format(color)), subpixel=True)
            sprite.scale = scale
            return sprite
        def object_sprite(name, scale=0.15/600.):
            sprite = pyglet.sprite.Sprite(centered_image('Driver/imgs/{}.png'.format(name)), subpixel=True)
            sprite.scale = scale
            return sprite
    ...
    def run_modified(self, history_x, history_u):
        ...
        pyglet.clock.schedule_interval(self.animation_loop, 0.04)
        pyglet.clock.schedule_interval(self.control_loop, self.dt)
        self.autoquit = True
        self.paused = False
        self.event_loop.run()
        ...
        data = np.array([np.array(list(image_data.get_data('RGB', image_data.width*3))).reshape(image_data.height,image_data.width,3) for image_data in self.VIDEO])
        return np.transpose(np.uint8(data), (1,2,3,0))[::-1]
