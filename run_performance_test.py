...
def run_performance_test(planner_type):
    ...
    for _ in range(1):
        ...
        for w in weights:
            ...
            _ = planner.find_optimum(w, heuristic=True)
            opt_time = time.time() - t
            print('opt time', opt_time)
            planner.set_planner_param(100)
            print('comp ground truth path')
            traj_opt = planner.find_optimum(w, heuristic=True)
...
def visualize():
    ...
    for file in files:
        if 'csv' in file:
            print('file', file)
            df = df._append(pd.read_csv(path + "/" + file))
...
if __name__ == '__main__':
    run_performance_test('GraphMinMax')
    visualize()